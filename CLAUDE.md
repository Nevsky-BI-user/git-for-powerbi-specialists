...
docs/
  ...
  assets/engine.js      дані (PLAYERS/QUIZ/SCEN/QCHECKS/CSIM/ORDERS/TERMLAB/CMDANIM/UIMOCK/ICONS/GIT_CMDS) + віджети (вкл. termlab-рушій: стан репо, парсер git-команд, goal-DSL) + пошук + прогрес
...
- **Критичне правило цілісного коміту**: PBI-звіт (PBIP) — цілісна збірка файлів; норма курсу — комітити його ЦІЛКОМ (`git add .` → commit = знімок звіту). Частковий `git add <файл>` НЕ подавати як норму для одного звіту (ризик неконсистентного знімка) — лише як опцію для незалежних артефактів у репо (README, скрипти, інший звіт) і як механіку конфліктів/`add -p` у «Поглиблено». «Осмислені коміти» = окремі зміни В ЧАСІ, кожна комітить весь звіт. Не переписувати.
...
`<div class="gplayer" data-p="key">` покроковий SVG-граф · `<div class="scplayer" data-s="key">` мультирепо-сценарій · `<div class="quiz" data-q="bank">` банк вправ · `<div class="qcheck" data-qc="key">` міні-перевірка · `<div class="csim" data-cs="key">` симулятор команди · `<div class="order" data-o="key">` впорядкування кроків · `<div class="termlab" data-tl="key">` інтерактивний тренажер терміналу (вільне введення git-команд + жива SVG-схема стану репо) · `<div class="diffq" data-dq="key">` задача «прочитай diff» (файл + розфарбований diff + питання; дані у `DIFFQ`; `colorizeDiffPre` також авто-підсвічує +/− у `<pre>`-diff квізів) · `<div class="cmdanim" data-ca="key">` анімація «що робить команда»: термінал з подруковуванням + праве вікно-контекст (панелі explorer/files/repo/remote; дані у `CMDANIM`, DSL кроків type/out/fx/note/pause, fx-оп'и валідує QA G2e; кнопка ▶, prefers-reduced-motion → миттєвий фінал) · `<div class="uimock" data-um="key">` схематична SVG-ілюстрація «як це виглядає в реальній програмі» (дані у `UIMOCK`: title/cap/svg; валідує G2f) · `<span class="ticon" data-i="key">` іконка програми з `ICONS` (git/github/azuredevops/vscode/gitbash/sourcetree/powerbi/terminal/folder/file/pbip; варіант `.lg`) · `<div class="dobox">` action-бокс «ЗРОБИ ЗАРАЗ» (чистий CSS, без даних) — для місць, де читач мусить реально щось зробити на своєму ПК (відкрити термінал, зберегти .pbip тощо) · `<div class="lect">` репліка лектора (чистий CSS, бурштиновий блок з 🎓) — живий викладацький коментар: «зверни увагу», типові помилки студентів, досвід з реальних PBI-проєктів; ключові фрази `<b>`; 3–5 на модуль, звертання на «ти», не дублює сусідні note/dobox · `<div class="ytvideo" data-v="key">` відео українською (клік-фасад → офіційний `youtube-nocookie` embed із таймкодом; дані у `VIDEOS`, спільний ID/канал у константах `YT_*`). Дані — у відповідних об'єктах `engine.js`; ключ без даних = FAIL у QA.
